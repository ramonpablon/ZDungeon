...
public class PlayerPhisics : MonoBehaviour {
    ...
    private void OnCollisionStay(Collision coll)
    {
        if (coll.gameObject.CompareTag("LimitsCollider"))
        {
            ...
            if (Vector3.Distance(transform.position, otherPlayerPos.position) < .8f && otherPlayerPos.GetComponent<PlayerPhisics>().inCombat)
            {
                if (otherPlayerPos.position.x > coll.gameObject.transform.position.x)
                {
                    otherPlayerPos.transform.position = Vector3.Lerp(otherPlayerPos.transform.position, otherPlayerPos.transform.position + new Vector3(4, 0, 0), Time.deltaTime);
                }
                else if (otherPlayerPos.position.x < coll.gameObject.transform.position.x)
                {
                    otherPlayerPos.transform.position = Vector3.Lerp(otherPlayerPos.transform.position, otherPlayerPos.transform.position - new Vector3(4, 0, 0), Time.deltaTime);
                }
            }
    ...
    public void CollisionBetweenPlayers()
    {
        if (!collLimits)
        {
            ...
            if (Vector3.Distance(transform.position, otherPlayerPos.position) < .8f && otherPlayerPos.GetComponent<PlayerPhisics>().inCombat)
            {
                if (otherPlayerPos.position.x < transform.position.x)
                {
                    transform.position = Vector3.Lerp(transform.position, otherPlayerPos.transform.position + new Vector3(4, 0, 0), Time.deltaTime);
                }
                else if (otherPlayerPos.position.x > transform.position.x)
                {
                    transform.position = Vector3.Lerp(transform.position, otherPlayerPos.transform.position - new Vector3(4, 0, 0), Time.deltaTime);
        ...
        Physics.IgnoreCollision(otherPlayerPos.GetComponent<Collider>(), GameManager.limitsColliderR.GetComponent<Collider>(), !collLimits);
        Physics.IgnoreCollision(otherPlayerPos.GetComponent<Collider>(), GameManager.limitsColliderL.GetComponent<Collider>(), !collLimits);
    }
