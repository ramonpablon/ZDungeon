...
public class InputManager : PlayerPhisics {
    ...
    void Update () {
        ...
        #region Combat
            ...
            #region Block
        if (Input.GetButtonDown(playerPrefix + "_ButtonRT") && canBlock && !isDashing)
        {
            Blocking(true);
            anim.SetTrigger("Block");
        }
        else if (Input.GetButtonUp(playerPrefix + "_ButtonRT"))
            Blocking(false);
        ...
        if (Input.GetButtonDown(playerPrefix + "_ButtonB") && !isDashing && !isStun)
        {
            if(onground)
            {
                if(moveY > 0)
                ...
                if (!AC.blockBreak)
                {
                    if (!blocking)
                    {
                        ...
                        else
                        {
                            ...
                            StartCoroutine(_Knockback(Vector3.up, 400, 0));
                        }
    ...
    private IEnumerator _LockStun(bool lockMovement, bool timed, float delayTime, float lockTime)
    {
        ...
        if (timed)
        {
            if(!anim.GetCurrentAnimatorStateInfo(0).IsTag("Knockout"))
    ...
    private IEnumerator _HitStop(bool inBlocking, float timeBetweenHits)
    {
        while (triggerCollEnemy)
        {
            ...
            otherPlayerPos.gameObject.GetComponent<InputManager>().animatorSpeedValue = 0;
            yield return new WaitForSeconds(timeBetweenHits);
            otherPlayerPos.gameObject.GetComponent<InputManager>().animatorSpeedValue = 1;
